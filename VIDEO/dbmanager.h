#ifndef DBMANAGER_H
#define DBMANAGER_H

#include <QObject>
#include <QSqlDatabase>
#include <QSqlQuery>
#include <QSqlError>
#include <QDebug>

class DBManager : public QObject
{
    Q_OBJECT

public:
    explicit DBManager(QObject *parent = nullptr);
    ~DBManager();

    bool connectToDatabase(const QString &host, const QString &databaseName, const QString &user, const QString &password);
    void disconnectFromDatabase();
    bool insertSensorData(const QString &sensorType, float value);
    QStringList fetchSensorData(); // Nouvelle méthode pour récupérer les données des capteurs
    bool insertScreenshot(const QString &filename); // Méthode pour insérer une capture d'écran

private:
    QSqlDatabase db; // Instance de la base de données
};

#endif // DBMANAGER_H
